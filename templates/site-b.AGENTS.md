...
2. 确认 `.env` 包含 `SITE_B_SHARE`、`SITE_B_USERNAME`、`SITE_B_PASSWORD` 和 `SITE_B_PREVIEW_URL`。
...
9. 使用 `.env` 中的账号密码建立到 `SITE_B_SHARE` 的临时 Windows 网络连接。不永久保存网络映射。
...
14. 使用部署 ID 构造版本化预览地址：`SITE_B_PREVIEW_URL?v=<deploymentId>`。如果原地址已有查询参数，则使用 `&v=`。
15. 从 `SITE_B_PREVIEW_URL` 解析出协议、主机和端口，请求该站点的 `/.internal/deployment.json?v=<deploymentId>`，确认 HTTP 200 且返回的 `deploymentId` 与本次一致。
